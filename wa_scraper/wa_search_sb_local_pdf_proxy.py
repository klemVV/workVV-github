...
for (var i = 0; i < rows.length; i++) {
    ...
    if (!text.includes("FULFILLED")) continue;
...
def run_letter(letter="A", out_dir="./output_wa_combined", headless=False, 
    ...
    """
    Run the scraper for a single letter.

    Proxy selection priority:
      1) explicit `proxy` argument (single proxy)
      2) random choice from `proxy_list` if provided
      3) PROXY_URL from WEBSHARE_PROXY env var
      4) no proxy
    """
    ...
    # Decide which proxy to use for this run
    proxy_to_use = proxy
    if proxy_to_use is None:
        if proxy_list:
            proxy_to_use = random.choice(proxy_list)
            print(f"[PROXY] Using random proxy from list: {proxy_to_use}")
        else:
